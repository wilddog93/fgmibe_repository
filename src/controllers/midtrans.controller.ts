...
const createCheckoutMemberSnap = catchAsync(async (req: Request, res: Response) => {
  const payload = req.body;
  const data = await checkoutService.checkoutRegisterMemberSnap(payload);
  res.status(httpStatus.OK).send(data);
});
