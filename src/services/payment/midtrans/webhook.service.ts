...
import { verifyNotificationSignature } from './service';
...
const handleMembershipRegistration = async (
  ...
  if (existing) {
    if (existing.status !== status) {
      ...
      if (status === 'COMPLETED' && !updated.memberId && cache) {
        const result = await prisma.$transaction(async (tx) => {
          const { member } = await memberService.createMemberAndUser(tx, cache);
  ...
  const result = await prisma.$transaction(async (tx) => {
    const { member, user } = await memberService.createMemberAndUser(tx, cache);
